...
//***********************************************
/*int rom_initWIFI(unsigned char * auxPointer)
{//Asignamos el puntero a la RAM del buffer
  char buf[17];
  bytes = auxPointer;  
  int type, bank_index, ram, region, version, i, pass;
  unsigned char checksum = 0;
  printf("ROM WIFI\n");
  if(memcmp(&auxPointer[0x104], header, sizeof(header)) != 0)
    return 0;  
  
  memcpy(buf, &auxPointer[0x134], 16);
  buf[16] = '\0';
  printf("Rom title: %s\n", buf);  
  type = auxPointer[0x147];
  bank_index = auxPointer[0x148];
  ...
  region = auxPointer[0x14A];
  if(region > 2)
    region = 2;
  printf("Region: %s\n", regions[region]);

  version = auxPointer[0x14C];
  printf("Version: %02X\n", version);

  for(i = 0x134; i <= 0x14C; i++)
    checksum = checksum - auxPointer[i] - 1;

  pass = auxPointer[0x14D] == checksum;
